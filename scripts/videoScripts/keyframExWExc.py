...
def process_all_videos():
    ...
    for video_id, relative_video_path in videos:
        ...
        # Open a text file to store frame index and timestamp
        with open(f'{keyframes_dir}/keyframe_timestamps_{video_id}.txt', 'w') as timestamp_file:
            if not scene_changes:
                ...
                while cap.isOpened():
                    ...
                    if index % keyframe_interval == 0:
                        ...
                        timestamp = index / fps
                        timestamp_file.write(f'Frame: {index}, Timestamp: {timestamp:.2f} seconds\n')
                    index += 1
            else:
