...
import clip
import sys
sys.path.append('D:/Simon/Uni/Summer24/IVAD/video_analysis/src/backend')
...
def get_keyframes(video_id):
    conn = load_database()
    cursor = conn.cursor()
    cursor.execute('SELECT frame_index, analysis_results, timestamp FROM Keyframes WHERE video_id = ?', (video_id,))
...
if query:
    ...
    if video_results:
        ...
        for video in video_results:
            ...
            if keyframe_results:
                for frame_index, analysis_results, timestamp in keyframe_results:
                    frame_path = get_keyframe_image(video_id, frame_index)
                    if os.path.exists(frame_path):
                        analysis_results = np.frombuffer(analysis_results, dtype=np.float16)
                        st.image(frame_path, caption=f"Keyframe {frame_index} - Analysis: {analysis_results} - Timestamp: {timestamp}")
                    else:
...
if uploaded_file is not None:
    ...
    for vid, idx, sim in similar_keyframes:
        ...
        if os.path.exists(frame_path):
            conn = load_database()
            cursor = conn.cursor()
            cursor.execute('SELECT timestamp FROM Keyframes WHERE video_id = ? AND frame_index = ?', (vid, idx))
            timestamp = float(cursor.fetchone()[0])
            conn.close()

            # Convert timestamp to minutes and seconds for simplicity in the UI
            minutes, seconds = divmod(timestamp, 60)
            formatted_timestamp = f"{int(minutes):02d}:{seconds:05.2f}"

            milliseconds = int(timestamp * 1000)
            formatted_milliseconds = f"{milliseconds:05d}"
            
            st.image(frame_path, caption=f"ID: {vid}, Frame Index: {idx}, Similarity: {sim:.4f}")
            st.write(f"**Timestamp:** {formatted_timestamp} mins / {formatted_milliseconds} ms")
            
            video_path = get_video_path(vid)
            video_url = f"{video_path}"
            st.video(video_url, format="video/mp4,video/quicktime")
        else:
